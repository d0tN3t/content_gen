import sys,pdb,tweepy,time,os,pickle,gnp,json,newspaper,requests,random,string,xmlrpclib,datetime,language_check,nltk,operator,urllib
...
###########################################################################################################################################################
def iterateoverplaces():
	placeids=[23424975,23424977,23424748,23424916,23424775,23424942,23424948,23424803,23424848,	23424768, 23424851,\
	 23424901,23424846,23424747]
	countries=['uk','us','aus','newzealand','canada','southafrica','singapore','ireland','india','brazil','iran', \
	'Malaysia', 'Indonesia' 'Argentina' ]

def get_trends_twitter(place_id):
...
def checkfeasibletrend(new_trends):
	for trend in new_trends:
		...
		if len(urls) > 5:
			...
			write_post_to_wordpress(summary,title,keywords)
...
def replacesynonym(text):
	...
	for word in c:
		...
		if word not in stops and syn:
			syns_words=syn[0].lemma_names()
			outputty+=' '+syns_words[0]
		else:
			outputty +=' ' +word
	return outputty
...
####################################################################################################################################################33
# this will remove the current trends_till_now.p
if False:
	current_path=os.path.dirname(os.path.realpath(__file__))
	current_path=current_path+'\\trends_till_now.p'
	if os.path.exists(current_path):
...
latest_trends=get_trends_twitter(None)
new_trends=store_trends(latest_trends)
checkfeasibletrend(new_trends)

# This is will make it run inifinitely, It will keep checking if a new trend is available.
# and it will posts only if there is.
# while True:
# 	latest_trends=get_trends_twitter(None)
# 	new_trends=store_trends(latest_trends)
# 	checkfeasibletrend(new_trends)